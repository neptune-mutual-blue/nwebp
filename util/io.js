...
const findFilesRecursive = async (extensions, directoryName, results = []) => {
  ...
  for (const f of files) {
    ...
    if (f.isDirectory()) {
      await findFilesRecursive(extensions, fullPath, results)
    } else {
      if (extensions.indexOf(fullPath.split('.').pop()) > -1) {
...
const findFiles = async (extensions, directoryName, results = []) => {
  const files = await fs.readdir(directoryName, { withFileTypes: true })
...
module.exports = { saveToDisk, deleteFile, ensureDirectory, ensureFile, cacheValue, fetchValue, findFilesRecursive, readFile, exists, findFiles }
